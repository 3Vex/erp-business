    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            <div class="text-center">
                ...
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Create a new account</p>
            </div>

            <form @submit.prevent="handleRegister" class="mt-8 space-y-6 bg-white dark:bg-gray-800 p-8 rounded-lg shadow">
...
async function handleRegister() {
    ...
    loading.value = true;
    try {
        await authStore.register({ name: name.value, email: email.value, password: password.value, password_confirmation: passwordConfirmation.value });
