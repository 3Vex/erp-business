...
export const useAgentsStore = defineStore('agents', () => {
  ...
  async function createAgent(payload: Partial<Agenofile>): Promise<Agenofile | null> {
    try {
      const { data } = await axios.post('/api/v1/agents', payload)
      const agent = data.data as Agenofile
  ...
  async function updateAgent(id: number, payload: Partial<Agenofile>): Promise<boolean> {
    try {
