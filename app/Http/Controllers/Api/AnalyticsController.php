...
class AnalyticsController extends BaseApiController
{
    ...
    public function charts(Request $request): JsonResponse
    {
        ...
        $revenueTrend = DB::table('sales_orders')
            ...
            ->selectRaw($this->dateFormatSql('created_at') . " as month, sum(total_amount) as value")
            ->groupBy('month')
            ->orderBy('month')
            ->get();
        ...
        $ordersTrend = DB::table('sales_orders')
            ->where('created_at', '>=', now()->subMonths($months))
            ->selectRaw($this->dateFormatSql('created_at') . " as month, count(*) as value")
            ->groupBy('month')
            ->orderBy('month')
            ->get();

        $fleetFuelCost = FuelLog::where('date', '>=', now()->subMonths($months)->toDateString())
            ->selectRaw($this->dateFormatSql('date') . " as month, sum(total_cost) as value")
            ->groupBy('month')
            ->orderBy('month')
            ->get();
        ...
        $fleetMaintenanceCost = MaintenanceRecord::where('status', 'completed')
            ...
            ->selectRaw($this->dateFormatSql('completed_date') . " as month, sum(cost) as value")
            ->groupBy('month')
            ->orderBy('month')
            ->get();
