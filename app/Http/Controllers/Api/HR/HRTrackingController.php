...
class HRTrackingController extends BaseApiController
{
    public function dashboard(Request $request): JsonResponse
    {
        ...
        $monthlyHires = Employee::where('created_at', '>=', now()->subMonths(12))
            ->selectRaw($this->dateFormatSql('created_at') . " as month, count(*) as count")
            ->groupBy('month')
            ->orderBy('month')
            ->get();
    ...
    public function leaveReport(Request $request): JsonResponse
    {
        ...
        $monthlyTrend = (clone $query)
            ->selectRaw($this->dateFormatSql('start_date') . " as month, count(*) as count, sum(total_days) as days")
            ->groupBy('month')
            ->orderBy('month')
            ->get();
    ...
    public function payrollReport(Request $request): JsonResponse
    {
        ...
        $monthlyTrend = (clone $query)
            ->selectRaw($this->dateFormatSql('hr_payroll.created_at') . " as month, sum(net_salary) as total, count(*) as count")
            ->groupBy('month')
            ->orderBy('month')
            ->get();
